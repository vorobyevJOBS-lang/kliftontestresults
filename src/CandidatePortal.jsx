...
export default function CandidatePortal() {
  ...
  useEffect(() => { if (!state.assignment || done || response === (state.assignment.candidate_response || "")) return undefined; setDraftState("saving"); const timer = window.setTimeout(() => saveCandidateAssignmentDraft(token, response).then(() => setDraftState("saved")).catch(() => setDraftState("error")), 900); return () => window.clearTimeout(timer); }, [token, response, done, state.assignment]);
  ...
  return <div className="eh-shell"><header className="eh-header"><div className="eh-header-inner"><div className="eh-brand"><span className="eh-brand-mark">E</span><span>EvidenceHire</span></div><span className="eh-family">Задание кандидата</span></div></header><main className="eh-main" style={{ maxWidth: 780 }}>
    <div className="eh-panel"><span className="eh-family">{profile.family}</span><h1>{profile.workSample.title}</h1><p style={{ color: "#647068", lineHeight: 1.6 }}>Здравствуйте, {state.assignment.candidate_name}. Это короткая рабочая проба для роли «{profile.name}». Ожидаемое время — около {profile.workSample.minutes} минут.</p>
      ...
      {done ? <div className="eh-empty"><h2>Ответ получен</h2><p>Спасибо. Команда найма рассмотрит решение по одинаковой для всех кандидатов рубрике.</p></div> : <>
        <div className="eh-progress-line" aria-label="Прогресс ответа"><div className="eh-progress-track"><span style={{ width: `${Math.min(100, Math.max(8, response.trim().length / 8))}%` }} /></div><div className="eh-progress-label"><span>{response.trim().length} символов</span><strong>{draftState === "saving" ? "Сохраняем…" : draftState === "saved" ? "Черновик сохранён" : draftState === "error" ? "Не удалось сохранить" : ""}</strong></div></div>
        <label className="eh-label" htmlFor="candidate-response" style={{ marginTop: 16 }}>Ваш ответ</label><textarea id="candidate-response" className="eh-textarea" style={{ minHeight: 260 }} maxLength={20000} value={response} onChange={(event) => setResponse(event.target.value)} placeholder="Опишите ход решения, ключевые предположения и следующий шаг." aria-describedby="draft-help" />
        <p id="draft-help" className="eh-helper">Черновик автоматически сохраняется в защищённой базе. Можно закрыть страницу и продолжить по той же ссылке до отправки.</p>
