...
function Profiles({ profiles, onSelect, canCreate }) {
  ...
  return <>
    ...
    <div className="eh-toolbar">
      <div><h2>Должности JOBS и Клячки</h2><p>{profiles.length} рабочих профилей · все пока проходят локальную проверку</p></div>
...
function CandidateForm({ profile, branches, onCancel, onCreate, onApprove, canApprove }) {
  ...
  const [review, setReview] = useState({ reviewers: "", criticalTasks: "", criticalErrors: "", entryRequirements: "", outcomeDefinition: "", sample: false, anchors: false, accommodations: false });
...
function Today({ candidates, onOpen, onNew, onCandidates, canCreate }) {
  if (!canCreate) {
  ...
  return <>
    ...
    {!priority.length ? <div className="eh-empty"><h3>Срочных действий нет</h3><p>{canCreate ? "Добавьте кандидата или откройте общую воронку." : "Откройте общую воронку, чтобы проверить назначенные карточки."}</p></div> : <div className="eh-candidates">{priority.map((item) => <button type="button" className="eh-candidate eh-candidate-button" key={item.id} onClick={() => onOpen(item)}><div><strong>{item.name}</strong><p>{PIPELINE_LABELS[item.pipelineStage || "new"] || "Активный этап"} · {item.nextAction || "Проверить карточку"}</p></div><span>Открыть →</span></button>)}</div>}
  </>;
}
