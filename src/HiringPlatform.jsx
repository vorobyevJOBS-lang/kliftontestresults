...
const ACTIVE_INVITE_STATUSES = new Set(["created", "opened", "in_progress"]);

function InviteBadge({ candidate }) {
  const status = INVITE_STATUS[candidate.inviteStatus || "none"] || INVITE_STATUS.none;
  return <span className={`eh-invite-status is-${status.tone}`}>{status.label}</span>;
}

function InviteProgressNotice({ candidate }) {
  const status = candidate.inviteStatus || "none";
  const date = (value) => value ? new Date(value).toLocaleString("ru-RU", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }) : "";
  const messages = {
    none: "Персональная ссылка ещё не создана.",
    created: `Ссылка создана, но кандидат ещё не открыл её${candidate.inviteExpiresAt ? `. Действует до ${date(candidate.inviteExpiresAt)}` : ""}.`,
    opened: `Кандидат открыл задание ${date(candidate.inviteOpenedAt)} и ещё не сохранил черновик.`,
    in_progress: `Кандидат заполняет задание. Последнее сохранение черновика: ${date(candidate.inviteDraftUpdatedAt)}.`,
    submitted: `Ответ кандидата получен ${date(candidate.candidateSubmittedAt)} и готов к проверке.`,
    expired: "Срок ссылки истёк или предыдущая ссылка была заменена. Создайте новую ссылку, если кандидат продолжает отбор.",
  };
  return <div className={`eh-invite-notice is-${INVITE_STATUS[status]?.tone || "neutral"}`}><InviteBadge candidate={candidate} /><span>{messages[status]}</span></div>;
}
...
function CandidateForm({ profile, branches, onCancel, onCreate, onApprove, canApprove }) {
  ...
  return <div className="eh-panel">
    ...
    <div className="eh-form-grid">
      ...
      <div><label className="eh-label" htmlFor="candidate-email">Email кандидата — необязательно</label><input id="candidate-email" className="eh-input" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} type="email" autoComplete="email" /><p className="eh-helper">После создания откроется персональная ссылка и готовое письмо. Система не отправляет письмо без вашего нажатия.</p></div>
    ...
    <div className="eh-actions" style={{ marginTop: 22 }}><button className="eh-btn eh-btn-primary" type="button" disabled={profile.status === "draft" || !form.name.trim() || !form.branchId || submitting} onClick={create}>{submitting ? "Создаём ссылку…" : "Создать карточку и ссылку"}</button><button className="eh-btn eh-btn-ghost" type="button" onClick={onCancel}>Отмена</button></div>
    </>}
...
function Assessment({ candidate, profile, onChange, onChangeOutcome, onBack, onArchive, onRestore, onSave, onSaveOutcome, onCreateInvite, onSetCandidateModules, onSubmitRating, onAddNote, saveState, saveError, ratingState, canManageOutcomes, canManageCrm, canArchive, canDecide, canReviewSubmittedWithoutOwnRating, canInvite, demo }) {
  ...
  const [inviteState, setInviteState] = useState(candidate.freshInviteUrl
    ? { status: "ready", url: candidate.freshInviteUrl }
    : candidate.freshInviteError ? { status: "error", url: "" } : { status: "", url: "" });
  ...
  const requestInvite = async () => {
    if (ACTIVE_INVITE_STATUSES.has(candidate.inviteStatus) && !window.confirm("Текущая ссылка перестанет работать. Создать и скопировать новую ссылку?")) return;
    setInviteState({ status: "loading", url: "" });
    try {
      const link = await onCreateInvite();
      setInviteState({ status: "ready", url: link });
      try { await navigator.clipboard.writeText(link); } catch { /* The visible URL remains available for manual copy on Safari. */ }
    } catch { setInviteState({ status: "error", url: "" }); }
  };
  const inviteButtonLabel = candidate.inviteStatus === "expired" ? "Новая ссылка" : ACTIVE_INVITE_STATUSES.has(candidate.inviteStatus) ? "Заменить ссылку" : "Создать ссылку";
  const inviteSubject = `Задание для отбора в ${branchById(candidate.branchId).name}`;
  const inviteMessage = inviteState.url ? `Здравствуйте, ${candidate.name}!\n\nДля следующего этапа отбора откройте персональную ссылку и выполните задание в удобное время:\n${inviteState.url}\n\nСсылка действует 7 дней и предназначена только для вас. Если возникнет вопрос, ответьте на это сообщение.` : "";
  const mailtoUrl = candidate.email && inviteState.url ? `mailto:${encodeURIComponent(candidate.email)}?subject=${encodeURIComponent(inviteSubject)}&body=${encodeURIComponent(inviteMessage)}` : "";
  return <>
    <button type="button" className="eh-btn eh-btn-ghost eh-back" onClick={onBack}>← К кандидатам</button>
    <div className="eh-role-head"><div><span className="eh-family">{profile.family}</span><h1>{candidate.name}</h1><p>{profile.name} · {branchById(candidate.branchId).name} · создано {new Date(candidate.createdAt).toLocaleDateString("ru-RU")}</p></div><div className="eh-actions"><button type="button" className="eh-btn eh-btn-ghost eh-print-button" onClick={() => { setOpen({ sample: true, interview: true, decision: true, references: true, outcome: true }); setTimeout(() => window.print(), 50); }}>Печать</button>{canInvite && !candidate.archivedAt && !candidate.candidateSubmittedAt && <button type="button" className="eh-btn eh-btn-secondary" disabled={demo || inviteState.status === "loading"} onClick={requestInvite}>{inviteState.status === "loading" ? "Создаём…" : inviteButtonLabel}</button>}<button type="button" className="eh-btn eh-btn-primary" disabled={saveState === "saving" || Boolean(candidate.archivedAt)} onClick={() => onSave()}>{saveState === "saving" ? "Сохраняем…" : saveState === "saved" ? "Сохранено ✓" : "Сохранить"}</button>{canArchive && (candidate.archivedAt ? <button type="button" className="eh-btn eh-btn-secondary" onClick={onRestore}>Восстановить</button> : <button type="button" className="eh-btn eh-btn-danger" onClick={onArchive}>В архив</button>)}</div></div>
    ...
    {!candidate.archivedAt && <InviteProgressNotice candidate={candidate} />}
    {inviteState.status === "ready" && <div role="status" className="eh-callout" style={{ marginBottom: 16 }}><label className="eh-label" htmlFor="candidate-invite-link">Персональная ссылка готова и действует 7 дней</label><div className="eh-note-composer"><div><input id="candidate-invite-link" className="eh-input" readOnly value={inviteState.url} onFocus={(event) => event.target.select()} /><button type="button" className="eh-btn eh-btn-secondary" onClick={() => navigator.clipboard.writeText(inviteState.url).catch(() => {})}>Копировать</button></div></div><div className="eh-actions" style={{ marginTop: 12 }}>{mailtoUrl && <a className="eh-btn eh-btn-primary" href={mailtoUrl}>Открыть готовое письмо</a>}<button type="button" className="eh-btn eh-btn-secondary" onClick={async () => { if (navigator.share) { try { await navigator.share({ title: inviteSubject, text: inviteMessage }); return; } catch (reason) { if (reason?.name === "AbortError") return; } } await navigator.clipboard.writeText(inviteMessage).catch(() => {}); }}>Поделиться или скопировать текст</button></div><p className="eh-helper">Важно: письмо не отправляется автоматически. Нажмите «Открыть готовое письмо» и отправьте его либо поделитесь ссылкой в мессенджере.</p></div>}
    {inviteState.status === "error" && <div role="alert" className="eh-callout" style={{ marginBottom: 16 }}>Карточка кандидата сохранена, но ссылку создать не удалось. Нажмите «Создать ссылку» ещё раз — повторная карточка не нужна.</div>}
    ...
    <div className="eh-score-layout">
      <div className="eh-stack">
        <Stage index="1" title={profile.workSample.title} subtitle={`Рабочая проба · около ${profile.workSample.minutes} минут`} open={open.sample} onToggle={() => setOpen({ ...open, sample: !open.sample })}>
          <div className="eh-callout"><strong>Что сообщить кандидату до таймера</strong><br />{profile.workSample.candidateBrief || "Это проверка способа работы, а не знания внутренних регламентов. Выдайте все необходимые факты и разрешите кандидату задавать уточняющие вопросы."}</div>
...
function Today({ candidates, onOpen, onNew, onCandidates, onProfiles, canCreate, canApproveProfiles, pilotProfileCount }) {
  ...
  const filling = active.filter((item) => ["opened", "in_progress"].includes(item.inviteStatus));
  const expiredInvites = active.filter((item) => item.inviteStatus === "expired");
  const waitingDecision = active.filter((item) => item.pipelineStage === "decision" && (!item.finalDecision || item.finalDecision === "pending"));
  const priority = [...expiredInvites, ...overdue, ...submitted, ...waitingDecision].filter((item, index, rows) => rows.findIndex((row) => row.id === item.id) === index).slice(0, 8);
  ...
  return <>
    ...
    <div className="eh-metric-grid">
      ...
      <button type="button" onClick={onCandidates}><span>Заполняют</span><strong>{filling.length}</strong><small>открыли ссылку или сохранили черновик</small></button>
      <button type="button" onClick={onCandidates}><span>Ссылка истекла</span><strong>{expiredInvites.length}</strong><small>нужно решить, отправлять ли новую</small></button>
