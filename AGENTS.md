...
- Active pilots: `klyachka_enrollment_manager` version 3 and `klyachka_trial_sales_manager` version 3 are owner-confirmed. Version 3 separates the candidate briefing from the interviewer-only legend and forbids scoring prior knowledge of internal prices, scripts, CRM, or school procedures. Klyachka managers and HR can create candidates and send protected invitations for both roles; every other role remains a draft until its own job analysis is confirmed. Trial-sales outcome: same-day payment / actually conducted trials over the latest 30 days, with 10% low, 15% average and 20% good benchmarks.
- Work-preference results: after all 166 pairs, show role correspondence to the candidate and HR as a transparent comparative hypothesis. Display rank, matched themes, interview question, and role-specific work sample; never present the result as a success probability or automatic hiring verdict.
