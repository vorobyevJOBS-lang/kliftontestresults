...
- Active pilots: `klyachka_enrollment_manager` version 2 and `klyachka_trial_sales_manager` version 2 are owner-confirmed. Klyachka managers and HR can create candidates and send protected invitations for both roles; every other role remains a draft until its own job analysis is confirmed. Trial-sales outcome: same-day payment / actually conducted trials over the latest 30 days, with 10% low, 15% average and 20% good benchmarks.
